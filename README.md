...
| **Sinh cả ban ngày** khi đang trong đợt, dùng cơ chế sinh tự nhiên | Engine sinh dùng đúng `NaturalSpawner.isSpawnPositionOk` của vanilla + cho phép placement bỏ qua ánh sáng | `spawning.daylightSpawn`, `boostNaturalPlacement` |
...
| Sát thương nhiều → **màn hình mờ đỏ** | "Trauma" tích lũy theo máu mất, vignette đỏ có độ đục động, nhịp nhẹ, hồi dần | `damageOverlay.*` |
...
| **Config chỉnh mọi thứ trong game** (ấn Mod → Config) | Gần 60 khóa trong COMMON + CLIENT, mở qua màn hình cấu hình bản địa của NeoForge, có dịch EN/VI | `zombietide-*.toml` |
...
| `/zombietide config list [lọc]` | mọi ngưởi | Liệt kê ~60 khóa config |
...
```bash
./gradlew build      # → build/libs/zombietide-1.0.0.jar
